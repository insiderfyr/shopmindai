...
export default function useChatFunctions({
  ...
  const ask: TAskFunction = (
    ...
    if (!isRegenerate) {
      submission.messages.push(currentMsg);
    }

    // We need to update the messages list with the new user message and the initial response
    const newMessages = [...submission.messages, initialResponse];

    if (isRegenerate) {
      // When regenerating, we replace the last message with the new initial response
      const lastMessageIndex = submission.messages.length - 1;
      if (lastMessageIndex >= 0) {
        submission.messages[lastMessageIndex] = initialResponse;
